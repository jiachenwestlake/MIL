...
class ManualVerbalizer(Verbalizer):
    ...
    def project_prob(self, logits):
        vocab_probs_distribution = F.softmax(logits, dim=-1)
        # sorted_probs, sorted_index = torch.sort(vocab_probs_distribution, dim=-1, descending=True)
        # top_100_probs = sorted_probs[0][:100].cpu().tolist()
        # top_100_ids = sorted_index[0][:100].cpu().tolist()
        # print(top_100_probs)
        # print(self.tokenizer.convert_ids_to_tokens(top_100_ids))
        # exit(0)
        label_words_probs = vocab_probs_distribution[:, self.label_words_ids]
    ...
    def process_logits(self, logits: torch.Tensor, **kwargs):
        ...
        return label_logits, label_words_logits, label_words_probs
